...
def export_models_for_wasm(output_dir: Path):
    ...
    if vae_ckpt.exists():
        ...
        try:
            ...
            ddsp.load_state_dict(ckpt["ddsp"])
            print(f"[+] Loaded VAE checkpoint from {vae_ckpt}")
        except Exception as e:
            print(f"[*] Initialized VAE with fresh progressive weights ({e})")
    ...
    if mamba_ckpt.exists():
        ...
        try:
            ...
            meta.load_state_dict(ckpt["meta_controller"])
            print(f"[+] Loaded Mamba-2 checkpoint from {mamba_ckpt}")
        except Exception as e:
            print(f"[*] Initialized Mamba-2 with fresh progressive weights ({e})")
    ...
    for k in range(5):
        ...
        print(f"    Tier {k} ({slice_metadata[k]['description'][:15]}...) -> Cumulative {len(cum_data) / 1024 / 1024:.2f} MB | CRC32: {crc}")
    ...
    deploy_config = {
        ...
        "neural_control": {
            ...
            "meta_controller": {
                ...
                "inputs": ["moe_logits", "telemetry (4d)", "user_weights", "quality_scores", "active_slice_level"]
            }
        }
    }

    # Writes config to the base directory (crates/inference/data/)
    ...
    torch.save(encoder.state_dict(), output_dir / "encoder_weights.pt")
    torch.save(ddsp.state_dict(), output_dir / "ddsp_weights.pt")
    torch.save(mamba.state_dict(), output_dir / "mamba2_weights.pt")
    torch.save(meta.state_dict(), output_dir / "meta_controller_weights.pt")
    print(f"[+] All PyTorch full checkpoints exported to {output_dir}")

    print("\n" + "=" * 70)
    print("ROLE-ANCHORED PROGRESSIVE SLICE EXPORT COMPLETE")
    print("=" * 70)


if __name__ == "__main__":
    out_dir = PROJECT_ROOT.parent / "crates" / "inference" / "data" / "wasm"
