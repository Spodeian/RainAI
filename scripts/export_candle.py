...
def save_safetensors(tensors_dict: dict, file_path: Path, metadata: dict = None):
    """
    Saves a dictionary of numpy arrays to a valid Hugging Face .safetensors binary.
    Zero external dependencies, 100% specification compliant.
    """
    header = {}
    if metadata:
    ...
    print(f"[+] Exported Safetensors -> {file_path} ({file_path.stat().st_size / 1024:.1f} KB, {len(tensors_dict)} tensors)")
...
def export_models_to_candle(output_dir: Path):
    ...
    print("=" * 70)
    print("RainAI Candle Safetensors Export Pipeline")
    print("=" * 70)
...
if __name__ == "__main__":
    out_dir = PROJECT_ROOT.parent / "crates" / "inference" / "data" / "candle"
