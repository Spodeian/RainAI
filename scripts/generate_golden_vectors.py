...
def build_deterministic_matrices():
    ...
    # W (omni, monopole), X (front-back, dipole), Y (left-right, dipole), Z (up-down, dipole)
...
def simulate_recurrence_steps(a_diag, b_diag, w_in, b_in, w_gate, w_foa):
    ...
    for t in range(NUM_STEPS):
        # Synthetic time-varying conditioning vector
        t_f = float(t)
        conditioning = (
        ...
        # Expert activation modulation: active experts preserve state, inactive decay
...
def main():
    output_dir = Path("../crates/inference/data/golden_vectors")
    output_dir.mkdir(parents=True, exist_ok=True)
    target_path = output_dir / "baseline_step_trace.json"

    print(f"Generating SymPy Box-Cox validation references...")
    box_cox_table = generate_symbolic_box_cox_table()

    print(f"Synthesizing parameter matrices and stepping Mamba2-MoE recurrence...")
