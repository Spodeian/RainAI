...
impl BoxCoxDequantizer {
    ...
    pub fn dequantize_scalar(val: f32, bit_width: f32, lambda: f32) -> f32 {
        ...
        if (1.0 - lambda).abs() < 1e-3 {
            ...
            let p = 1.0 - lambda;
            let inner = (1.0 + abs_y * p).max(0.0);
            sign * (inner.powf(1.0 / p) - 1.0)
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_linear_quantization_levels() {
        // 1-bit => 2 levels: -1.0, 1.0 or 0, 1
        let deq_0 = BoxCoxDequantizer::dequantize_scalar(0.0, 1.0, 0.0);
        assert_eq!(deq_0, 0.0);

        let deq_1 = BoxCoxDequantizer::dequantize_scalar(1.0, 1.0, 0.0);
        assert_eq!(deq_1, 1.0);
    }

    #[test]
    fn test_log_manifold_continuity() {
        // Near lambda = 1.0 (Taylor series branch)
        let deq_taylor = BoxCoxDequantizer::dequantize_scalar(0.5, 8.0, 1.0);
        let deq_close = BoxCoxDequantizer::dequantize_scalar(0.5, 8.0, 0.999);
        assert!((deq_taylor - deq_close).abs() < 1e-2, "Taylor expansion must be C1 continuous");
    }

    #[test]
    fn test_ternary_dequantization() {
        let ternary = vec![-1i8, 0, 1, 1, -1];
        let gamma = 0.25;
        let mut out = vec![0.0; 5];
        BoxCoxDequantizer::dequantize_ternary(&ternary, gamma, &mut out);
        assert_eq!(out, vec![-0.25, 0.0, 0.25, 0.25, -0.25]);
    }

    #[test]
    fn test_precision_format_snapping() {
        assert_eq!(PrecisionFormat::from_continuous_bit_width(0.2), PrecisionFormat::Pruned);
        assert_eq!(PrecisionFormat::from_continuous_bit_width(1.58), PrecisionFormat::Ternary158);
        assert_eq!(PrecisionFormat::from_continuous_bit_width(2.0), PrecisionFormat::Int2);
        assert_eq!(PrecisionFormat::from_continuous_bit_width(4.0), PrecisionFormat::Int4);
        assert_eq!(PrecisionFormat::from_continuous_bit_width(6.0), PrecisionFormat::Int6);
        assert_eq!(PrecisionFormat::from_continuous_bit_width(8.0), PrecisionFormat::Int8);
        assert_eq!(PrecisionFormat::from_continuous_bit_width(16.0), PrecisionFormat::Int16);
        assert_eq!(PrecisionFormat::from_continuous_bit_width(32.0), PrecisionFormat::Fp32);
    }

    #[test]
    fn test_layer_role_optimal_unquantized_format() {
        assert_eq!(LayerRole::MambaStateSpaceRecurrence.optimal_unquantized_format(), PrecisionFormat::Bf16);
        assert_eq!(LayerRole::LatentBottleneck.optimal_unquantized_format(), PrecisionFormat::Posit16);
        assert_eq!(LayerRole::DenseProjection.optimal_unquantized_format(), PrecisionFormat::Fp16);
        assert_eq!(LayerRole::FilterCoefficients.optimal_unquantized_format(), PrecisionFormat::Fp32);
        assert_eq!(LayerRole::AmbisonicRotation.optimal_unquantized_format(), PrecisionFormat::Fp32);
        assert_eq!(LayerRole::MacroConditioning.optimal_unquantized_format(), PrecisionFormat::Posit8);
    }

    #[test]
    fn test_bf16_conversion() {
        let original = 1.0f32;
        let bf = BoxCoxDequantizer::encode_bf16(original);
        let decoded = BoxCoxDequantizer::decode_bf16(bf);
        assert_eq!(original, decoded);

        let pi = std::f32::consts::PI;
        let bf_pi = BoxCoxDequantizer::encode_bf16(pi);
        let dec_pi = BoxCoxDequantizer::decode_bf16(bf_pi);
        assert!((pi - dec_pi).abs() < 0.02);
    }

    #[test]
    fn test_posit16_conversion() {
        // Values in [-1.0, 1.0] normalized audio domain
        for &val in &[0.0f32, 0.25, -0.5, 0.75, 1.0, -1.0] {
            let encoded = BoxCoxDequantizer::encode_posit16(val);
            let decoded = BoxCoxDequantizer::decode_posit16(encoded);
            assert!((val - decoded).abs() < 1e-3, "Posit16 roundtrip failed for {val}: got {decoded}");
        }
    }
    ...
    fn test_equal_power_crossfade() {
        ...
        let tgt = vec![1.0f32; 128];
        let mut out = vec![0.0f32; 128];

        apply_equal_power_crossfade(&src, &tgt, &mut out);

        // At t = 0, weight is cos(0) = 1.0
        assert!((out[0] - 1.0).abs() < 1e-3);
        // At t = halfway, cos(pi/4) + sin(pi/4) = sqrt(2) ≈ 1.414 (correlated sources sum in voltage)
        // Energy: cos^2(pi/4) + sin^2(pi/4) = 0.5 + 0.5 = 1.0
        assert!((out[64] - std::f32::consts::SQRT_2).abs() < 0.05);
        // At end, sin(pi/2) = 1.0
        assert!((out[127] - 1.0).abs() < 1e-3);
    }
}
