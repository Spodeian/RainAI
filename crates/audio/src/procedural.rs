...
impl ProceduralSynthesizer {
    ...
    pub fn process_frame(&mut self, state: &RainState) -> FoaFrame {
        ...
        let z = (rain_master * 0.45 * (1.0 - state.weather.pitch_angle)) + side_z;
...
mod tests {
    ...
    fn test_subtractive_filterbank_16_bands() {
        let mut filterbank = SubtractiveFilterbank16::new(48000.0);
        ...
        // Verify drift is non-zero
        assert!(LEARNED_DRIFT[0] > 0.0);
        assert!(LEARNED_DRIFT[3] < 0.0);

        // Run 1000 samples through each filter to verify numerical stability
        let mut sample = 0.5f32;
        for _ in 0..1000 {
            sample = (sample * 1.05).sin();
            for filter in filterbank.filters.iter_mut() {
                let out = filter.process(sample);
                assert!(!out.is_nan(), "Filter output must not be NaN");
                assert!(!out.is_infinite(), "Filter output must not be infinite");
            }
        }
    }

    #[test]
    fn test_side_sounds_insects_birds_traffic() {
        let mut synth = ProceduralSynthesizer::new(48000.0);
        let mut state = RainState::default();
        state.is_playing = true;
        state.master_volume = 1.0;
        // Mute base rain and wind to isolate side sounds
        state.weather.intensity = 0.0;
        state.wind.speed = 0.0;

        // 1. Insects
        state.side_sounds.insect_density = 0.8;
        let mut insect_energy = 0.0f32;
        for _ in 0..500 {
            let f = synth.process_frame(&state);
            insect_energy += f.w.abs();
        }
        assert!(insect_energy > 0.1, "Insects must generate non-zero energy");
        state.side_sounds.insect_density = 0.0;

        // 2. Wet Road Traffic
        state.side_sounds.traffic_distance = 0.3;
        let mut traffic_energy = 0.0f32;
        for _ in 0..500 {
            let f = synth.process_frame(&state);
            traffic_energy += f.w.abs();
        }
        assert!(traffic_energy > 0.1, "Traffic spray must generate non-zero energy");
    }
}
