...
NUM_WIND_PARAMS = 4
NUM_SIDE_SOUNDS = 18
NUM_PHYSICAL_PARAMS = NUM_BASE_SLIDERS + NUM_SURFACES + NUM_WIND_PARAMS + NUM_SIDE_SOUNDS
TOTAL_CONDITION_DIM = CLAP_EMBED_DIM + NUM_PHYSICAL_PARAMS + 1
...
class RainSpatialDataset(Dataset):
    def __init__(
        ...
        target_manifest = Path(manifest_path) if manifest_path else (self.data_dir / "manifest.json")
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if "physical_params" in item_meta and len(item_meta["physical_params"]) == NUM_PHYSICAL_PARAMS:
            ...
            base_sliders = [
                intensity, 0.2, 0.5, float(item_meta.get("surface_idx", 3)) / 8.0,
                intensity * 0.6, 0.5, 0.8, 0.2, 0.3, 0.1
            ]
            surfaces = [0.0] * NUM_SURFACES
            surfaces[item_meta.get("surface_idx", 3)] = 1.0
            wind_dynamics = [0.2, 0.1, 0.1, 0.05]
            side_sounds = [0.0] * NUM_SIDE_SOUNDS
            physical_params = np.array(base_sliders + surfaces + wind_dynamics + side_sounds, dtype=np.float32)

        # 3. FOA Yaw Rotation Augmentation
        if self.augment_yaw:
            ...
            physical_params[2] = float((physical_params[2] + (yaw_angle / (2.0 * np.pi))) % 1.0)
        ...
        sample = {
            ...
            "surface_idx": torch.tensor(item_meta.get("surface_idx", 3), dtype=torch.long),
...
class ActiveLearningSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self):
        ...
        sampled_indices = np.random.choice(self.dataset_size, size=self.dataset_size, replace=True, p=probs)
    ...
    def __init__(
        ...
        if importance_weights is not None:
            for i, fpath in enumerate(dataset.audio_files):
                tag = dataset.metadata.get(fpath.stem, {}).get("surface_tag", "default")
