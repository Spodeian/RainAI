...
class AcousticCorruptionPipeline:
    ...
    def temporal_gaussian_blur(self, x: torch.Tensor, max_kernel_size: int = 31, sigma_range: Tuple[float, float] = (1.5, 6.0)) -> torch.Tensor:
        ...
        kernel = (kernel / torch.sum(kernel)).view(1, 1, -1).repeat(channels, 1, 1).to(dtype=x.dtype)
    ...
    def spectrogram_tf_blur(self, x: torch.Tensor, kernel_size: int = 5, sigma: float = 1.8) -> torch.Tensor:
        batch, channels, samples = x.shape
        # Force float32 to protect against complex half autocast limitations
        x_flat = x.view(batch * channels, samples).float()
        ...
        return blurred_audio.view(batch, channels, samples).to(dtype=x.dtype)
    ...
    def lossy_codec_emulation(self, x: torch.Tensor) -> torch.Tensor:
        ...
        cutoff_bin = int(num_freqs * np.random.uniform(0.55, 0.75))
        stft[:, cutoff_bin:, :] = 0.0
        
        mid_mask = (torch.rand(1, num_freqs, 1, device=x.device) > 0.18).float()
        stft = stft * mid_mask
        
        corrupted = torch.istft(stft, n_fft=n_fft, hop_length=hop_len, window=window, length=samples)
        return corrupted.view(batch, channels, samples).to(dtype=x.dtype)
