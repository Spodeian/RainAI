...
Turn-key pipeline providing single-command hardware-adaptive execution (Rust-First Data Pipeline).
...
def run_rust_binary(bin_name: str, args: list = None, logger=None) -> bool:
    """
    Executes a precompiled Rust workspace binary from target/release if present,
    or invokes `cargo run --bin <bin_name> --release`. Returns True on success.
    """
    args = args or []
    ...
    candidates = [
        ...
        PROJECT_ROOT.parent / "target" / "release" / exe_name,
        PROJECT_ROOT / "crates" / "target" / "release" / exe_name,
    ]
    for exe_path in candidates:
        if exe_path.exists():
            if logger:
                logger.info(f"[+] Executing native Rust binary: {exe_path.name}")
            subprocess.run([str(exe_path)] + args, check=True)
            return True

    if shutil.which("cargo"):
        try:
            if logger:
                logger.info(f"[*] Dispatching via Cargo: {bin_name}")
            ...
            if logger:
                logger.warning(f"[!] Cargo run for '{bin_name}' failed ({e}).")
    return False
...
def check_dataset_and_manifest(
    ...
    acoustic_manifest_path = processed_dir / "acoustic_manifest.json"
    ...
    # STEP 1: Ingestion
    if prepare_data or rebuild_data or not raw_rain_dir.exists() or len(list(raw_rain_dir.glob("*.mp3")) + list(raw_rain_dir.glob("*.ogg"))) == 0:
        logger.info("[*] Ingesting multi-source open audio dataset...")
        t0 = time.time()
        if not run_rust_binary("rainai_ingest", logger=logger):
            logger.error("[!] Rust binary 'rainai_ingest' failed. Run `cargo build --release`.")
            sys.exit(1)
        logger.info(f"[+] Multi-source ingestion completed in {time.time() - t0:.2f}s.")
    ...
    if need_synthesis or prepare_data:
        logger.info("[*] Synthesizing physical rain acoustic corpus...")
        t0 = time.time()
        if not run_rust_binary("rainai_synth", logger=logger):
            logger.error("[!] Rust binary 'rainai_synth' failed.")
            sys.exit(1)
        logger.info(f"[+] Physical rain corpus synthesized in {time.time() - t0:.2f}s.")

    # STEP 3: Upmixing / Preprocessing
    ...
    if need_preprocess:
        logger.info(f"[*] Upmixing and preprocessing raw audio into 48kHz FOA chunks in {processed_dir}...")
        t0 = time.time()
        if not run_rust_binary("rainai_upmix", logger=logger):
            logger.error("[!] Rust binary 'rainai_upmix' failed.")
            sys.exit(1)
        logger.info(f"[+] Spatial preprocessing completed in {time.time() - t0:.2f}s.")

    # STEP 4: Feature Extraction & Manifest Generation
    manifest_data = {}
    target_manifest = acoustic_manifest_path if acoustic_manifest_path.exists() else manifest_path
    ...
    if len(manifest_data) < len(processed_files) or len(manifest_data) == 0 or rebuild_data:
        ...
        t0 = time.time()
        if not run_rust_binary("rainai_features", logger=logger):
            logger.error("[!] Rust binary 'rainai_features' failed.")
            sys.exit(1)
        
        # Load newly generated manifest
        target_manifest = acoustic_manifest_path if acoustic_manifest_path.exists() else manifest_path
        with open(target_manifest, "r", encoding="utf-8") as f:
            manifest_data = json.load(f)
        logger.info(f"[+] Acoustic features extracted in {time.time() - t0:.2f}s.")
    ...
    logger.info(f"Dataset Verified: {target_manifest.name} ({total_clips} chunks, {total_minutes:.1f} mins of 48kHz FOA audio)")
...
def run_phase_vae(logger: RainAILogger, python_exe: str, args, device: str, use_amp: bool):
    logger.stage(2, 5, "Phase 2: Spatial VAE + HOA-DDSP Engine Training (Compiled)")
...
def run_phase_mamba(logger: RainAILogger, python_exe: str, args, device: str, use_amp: bool):
    logger.stage(3, 5, "Phase 3: Mamba-2 MoE + Meta-Controller Training (Compiled)")
...
def run_phase_export(logger: RainAILogger, python_exe: str):
    ...
    subprocess.run([python_exe, "-X", "utf8", str(PROJECT_ROOT / "src" / "export" / "export_wasm.py")], check=True)

    logger.info("Exporting Hugging Face Candle Safetensors...")
    subprocess.run([python_exe, "-X", "utf8", str(PROJECT_ROOT / "src" / "export" / "export_candle.py")], check=True)

    logger.info("Exporting ONNX Computation Graphs & INT8 Quantized Models...")
    subprocess.run([python_exe, "-X", "utf8", str(PROJECT_ROOT / "src" / "export" / "export_onnx.py")], check=True)
...
def run_phase_benchmark(logger: RainAILogger, python_exe: str, iterations: int = 50):
    ...
    t0 = time.time()
    cmd = [
        python_exe, "-X", "utf8", str(PROJECT_ROOT / "src" / "benchmarks" / "benchmark_backends.py"),
...
def print_summary_table(logger: RainAILogger, total_time: float, phase_times: dict):
    ...
    exp_dir = PROJECT_ROOT / "crates" / "inference" / "data"
    if exp_dir.exists():
...
def main():
    ...
    if args.profile == "smoke-test":
        ...
        args.max_batches = args.max_batches if args.max_batches is not None else 0
        args.batch_size = args.batch_size or hw["recommended_batch_size"]
        args.accumulation_steps = args.accumulation_steps or hw["recommended_accumulation"]
        if args.use_amp is None:
            args.use_amp = hw["recommended_amp"]
    ...
    if args.rust_engine:
        logger.info("[+] Dispatching to Pure Rust Native Candle Training Engine...")
        rust_args = [
            "--phases", *phases,
            "--vae-epochs", str(args.epochs_vae),
            "--mamba-epochs", str(args.epochs_mamba),
            "--batch-size", str(args.batch_size),
            "--max-batches", str(args.max_batches),
            "--accumulation-steps", str(args.accumulation_steps),
        ]
        t0 = time.time()
        success = run_rust_binary("rainai_train_candle", rust_args, logger=logger)
        if not success:
            logger.error("[!] Failed running native Rust Candle training engine.")
            sys.exit(1)
        phase_times["Native Rust Candle Training"] = time.time() - t0
